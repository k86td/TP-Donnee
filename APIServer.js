...
module.exports =
    class APIServer {
        ...
        startupMessage() {
            log("**********************************");
            log("* API SERVER - version beta      *");
            log("**********************************");
            log("* Author: Nicolas Chourot        *");
            log("* Lionel-Groulx College          *");
            log("* Release date: august 30 2022   *");
            log("**********************************");
            log(`HTTP Server running on port ${this.port}...`);
            this.showMemoryUsage();
        }
        showRequestInfo() {
            let time = require('date-and-time').format(new Date(), 'YYYY MMMM DD - HH:mm:ss');
            log('<-------------------------', time, '-------------------------');
            log(`Request --> [${this.httpContext.req.method}::${this.httpContext.req.url}]`);
        ...
        showRequestProcessTime() {
            ...
            log("Response time: ", Math.round((requestProcessEndTime[0] * 1000 + requestProcessEndTime[1] / 1000000) / 1000 * 10000) / 10000, "seconds");
        }
        showMemoryUsage() {
            // for more info https://www.valentinog.com/blog/node-usage/
            const used = process.memoryUsage();
            log("Memory usage: ", "RSet size:", Math.round(used.rss / 1024 / 1024 * 100) / 100, "Mb |",
